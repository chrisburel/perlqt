...
void WriteIconInitialization::acceptUI(DomUI *node)
{
    ...
    output << option.indent << option.indent << option.indent << "return Qt4::Pixmap.new\n";
...
void WriteIconInitialization::acceptImage(DomImage *image)
{
    ...
    if (fmt == QLatin1String("XPM.GZ")) {
        output << option.indent << option.indent << option.indent << "return " << "Qt4::Pixmap.new(" << imageData << ")\n";
    } else {
        ...
        output << option.indent << option.indent << option.indent << 
                "return Qt4::Pixmap.fromImage(img)\n";
    }
}
