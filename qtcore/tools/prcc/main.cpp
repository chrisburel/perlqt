...
{
    fprintf(stderr, "PerlQt4 resource compiler\n");
    if (!error.isEmpty())
...
int runRcc(int argc, char *argv[])
{
    ...
    for (int i = 1; i < args.count() && errorMsg.isEmpty(); i++) {
        ...
        if (args[i][0] == QLatin1Char('-')) {   // option
            ...
            if (opt == QLatin1String("-o")) {
                ...
                fprintf(stderr, "PerlQt4 Resource Compiler version %s\n", QT_VERSION_STR);
                return 1;
    ...
    if (library.verbose())
        errorDevice.write("PerlQt4 resource compiler\n");
    ...
    if ((library.format() == RCCResourceLibrary::C_Code) ||
        (library.format() == RCCResourceLibrary::Perl_Code))
        mode |= QIODevice::Text;
