...
void WriteIncludes::acceptUI(DomUI *node)
{
    ...
    QString qualifiedClassName = node->elementClass() + m_uic->option().postfix;
    ...
    m_output << "package " << m_uic->option().prefix << className << ";\n";
    m_output << "\n";
...
void WriteIncludes::acceptCustomWidget(DomCustomWidget *node)
{
    ...
    if (!node->elementHeader() || node->elementHeader()->text().isEmpty()) {
        // no header specified, but it doesn't matter, since we just use the
        // className in Perl.
        add(className, true, className, 0);
    } else {
        ...
        add(className, true, className, global);
    }
}
