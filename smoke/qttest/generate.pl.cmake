...
my $qttest_headerlist = "";
my $qttest_headerlistpath = "";

$qttest_headerlist = "@CMAKE_CURRENT_SOURCE_DIR@/qttest_header_list";
$qttest_headerlistpath = "$here/$qttest_headerlist";

## If srcdir != builddir, use headerlist from src
$headerlistpath = $headerlist if ($headerlist =~ /^\//);
$qttest_headerlistpath = $qttest_headerlist if ($qttest_headerlist =~ /^\//);
...
open(HEADERS, $qttest_headerlistpath) or die "Couldn't open $qttest_headerlistpath: $!\n";
map { chomp ; $includes{$_} = 1 } <HEADERS>;
close HEADERS;
...
# Launch kalyptus
chdir "../smoke/qttest";
system "perl -I@perlqt4_SOURCE_DIR@/kalyptus @perlqt4_SOURCE_DIR@/kalyptus/kalyptus @ARGV --qt4 --globspace -fsmoke --name=qttest --classlist='@CMAKE_CURRENT_SOURCE_DIR@/classlist' --init-modules=qt $macros --no-cache --outputdir=$outdir @headers";
my $exit = $? >> 8;
exit $exit if ($exit);
chdir "$kalyptusdir";
